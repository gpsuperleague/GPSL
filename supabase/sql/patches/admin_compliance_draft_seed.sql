...
-- =============================================================================
-- Places real draft auction opening bids at market value (not instant signings).
-- Auctions settle via the normal transfer engine at random finish.
-- =============================================================================
...
DROP FUNCTION IF EXISTS public.admin_compliance_draft_seed_bids(text, integer, boolean, jsonb, jsonb);
DROP FUNCTION IF EXISTS public.admin_compliance_draft_seed_bids(text, int, boolean, jsonb, jsonb);
...
BEGIN
  ...
  IF now() >= v_bounds.draft_cutoff THEN
    RETURN jsonb_build_object('ok', false, 'reason', 'new_auction_locked_after_cutoff');
  END IF;
  ...
  FOR v_i IN 1..v_loop_max LOOP
    ...
    ELSE
      IF v_band_lo IS NOT NULL THEN
        v_pick_modes := ARRAY['pos', 'star', 'hg', 'u21', 'band', 'any'];
      ELSE
        v_pick_modes := ARRAY['pos', 'star', 'hg', 'u21', 'any'];
      END IF;
    END IF;
    ...
    IF v_window_bids > 0 THEN
      v_skipped := v_skipped || jsonb_build_array(
        jsonb_build_object(
          'reason', 'auction_already_open',
          'player_id', v_player.player_id,
          'player_name', v_player.player_name
        )
      );
...
GRANT EXECUTE ON FUNCTION public.admin_compliance_draft_seed_bids(text, integer, boolean, jsonb, jsonb) TO authenticated;
