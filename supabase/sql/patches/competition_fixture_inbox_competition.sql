...
BEGIN
  ...
  PERFORM public.match_schedule_notify_opponent(
    ...
    v_club || ' dropped the agreed time (24h+ notice). Propose a new kick-off on the schedule page.',
    v_opponent,
...
BEGIN
  ...
  PERFORM public.match_schedule_notify_opponent(
    ...
    v_club || ' used an emergency drop (<24h). The match is back on the schedule page.',
    v_opponent,
...
BEGIN
  ...
  v_body := public.competition_fixture_inbox_body(
    p_fixture_id,
    format('%s forfeited 3–0. %s', p_loser_club, coalesce(p_reason, ''))
  );
