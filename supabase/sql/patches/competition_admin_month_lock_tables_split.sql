-- =============================================================================
-- Fix: February (and other) month-lock "tables" stage timeout
--
-- Cause: manager_of_the_month.sql wrapped competition_admin_run_month_lock_jobs
-- so stage=tables ALSO ran competition_stadium_sync_all_clubs + MotM in the
-- same RPC. That exceeds the API gateway timeout → HTTP 500 / "tables timed out".
--
-- Fix:
--   • tables stage = Discord league-table queue only
--   • stadium stage = fill sync only
--   • motm stage = Manager of the Month only
-- Safe re-run. Then Retry month-lock jobs for February.
-- =============================================================================

-- Keep core runner: tables without stadium sync; add stadium / motm stages
...
DECLARE
  ...
  v_id bigint;
BEGIN
  -- API gateway often kills ~60s; keep DB timeout higher but stages must stay light
  ...
  ELSIF v_stage = 'totm' THEN
    v_do_totm := true; v_do_sport := false; v_do_tv := false;
    v_do_tables := false; v_do_stadium := false; v_do_motm := false;
    v_do_playoffs := false; v_do_clinches := false; v_do_scheduling := false;
  ELSIF v_stage = 'sport' THEN
    v_do_totm := false; v_do_sport := true; v_do_tv := false;
    v_do_tables := false; v_do_stadium := false; v_do_motm := false;
    v_do_playoffs := false; v_do_clinches := false; v_do_scheduling := false;
  ELSIF v_stage = 'tv' THEN
    v_do_totm := false; v_do_sport := false; v_do_tv := true;
    v_do_tables := false; v_do_stadium := false; v_do_motm := false;
    v_do_playoffs := false; v_do_clinches := false; v_do_scheduling := false;
  ELSIF v_stage IN ('tables', 'league_tables') THEN
    -- Discord standings queue ONLY
    v_do_totm := false; v_do_sport := false; v_do_tv := false;
    v_do_tables := true; v_do_stadium := false; v_do_motm := false;
    v_do_playoffs := false; v_do_clinches := false; v_do_scheduling := false;
  ELSIF v_stage IN ('stadium', 'attendance', 'stadium_fill') THEN
    v_do_totm := false; v_do_sport := false; v_do_tv := false;
    v_do_tables := false; v_do_stadium := true; v_do_motm := false;
    v_do_playoffs := false; v_do_clinches := false; v_do_scheduling := false;
  ELSIF v_stage IN ('motm', 'manager_of_month') THEN
    v_do_totm := false; v_do_sport := false; v_do_tv := false;
    v_do_tables := false; v_do_stadium := false; v_do_motm := true;
    v_do_playoffs := false; v_do_clinches := false; v_do_scheduling := false;
  ELSIF v_stage = 'playoffs' THEN
    v_do_totm := false; v_do_sport := false; v_do_tv := false;
    v_do_tables := false; v_do_stadium := false; v_do_motm := false;
    v_do_playoffs := true; v_do_clinches := false; v_do_scheduling := false;
  ELSIF v_stage = 'clinches' THEN
    v_do_totm := false; v_do_sport := false; v_do_tv := false;
    v_do_tables := false; v_do_stadium := false; v_do_motm := false;
    v_do_playoffs := false; v_do_clinches := true; v_do_scheduling := false;
  ELSIF v_stage IN ('scheduling', 'fines') THEN
    v_do_totm := false; v_do_sport := false; v_do_tv := false;
    v_do_tables := false; v_do_stadium := false; v_do_motm := false;
    v_do_playoffs := false; v_do_clinches := false; v_do_scheduling := true;
  ELSE
    RETURN jsonb_build_object(
      'ok', false,
      'reason', 'bad_stage',
      'hint', 'Use totm|sport|tv|tables|stadium|motm|playoffs|clinches|scheduling|all'
    );
  END IF;
  ...
  -- Resolve locked month once if needed
  IF v_month IS NULL AND (v_do_totm OR v_do_tables OR v_do_motm OR v_do_playoffs) THEN
  ...
  IF v_do_motm THEN
    BEGIN
      IF v_month IS NULL THEN
        v_motm := jsonb_build_object('ok', false, 'reason', 'no_locked_month');
      ELSIF to_regprocedure('public.competition_compute_manager_of_month(bigint,text)') IS NOT NULL THEN
        v_motm := public.competition_compute_manager_of_month(p_season_id, v_month);
      ELSE
        v_motm := jsonb_build_object('skipped', true, 'reason', 'motm_rpc_missing');
      END IF;
...
-- Thin admin wrapper: NO extra stadium/MotM on tables stage
