-- =============================================================================
-- Fix: nation pool cache refresh timeout (57014) via PostgREST / UI button
--
-- Full-table rebuild often exceeds the API statement_timeout. Split into
-- nation batches the UI can loop, each finishing in a few seconds.
--
-- Run this whole file once in Supabase SQL Editor.
--
-- UI (nation_player_pool / admin) will call:
--   international_refresh_nation_player_pool_cache_batch(p_limit, p_start)
-- SQL Editor one-shot still works:
...
-- Faster rating-band helper (single numeric eval; avoids per-row CTE rescans)
...
AS $$
  SELECT CASE
    ...
    WHEN v <= 65 THEN 'le_65'
    WHEN v <= 69 THEN 'r66_69'
    WHEN v <= 72 THEN 'r70_72'
    WHEN v <= 75 THEN 'r73_75'
    WHEN v <= 78 THEN 'r76_78'
    ELSE 'r79_plus'
  END
  FROM (
    SELECT nullif(
...
AS $function$
DECLARE
  v_limit integer := greatest(1, least(coalesce(nullif(p_limit, 0), 25), 40));
...
BEGIN
  ...
  -- Stay under typical PostgREST ~60s gateway
  ...
  IF coalesce(p_start, false) THEN
    ...
    SET refresh_run_at = v_run_at
    WHERE id = 1;
  ELSE
    SELECT m.refresh_run_at INTO v_run_at
...
-- SQL Editor one-shot: loop batches locally (ok with raised timeout)
...
BEGIN
  ...
  v_result := public.international_refresh_nation_player_pool_cache_batch(40, true);
  i := 0;
  WHILE NOT coalesce((v_result->>'done')::boolean, false) AND i < 80 LOOP
    i := i + 1;
    v_result := public.international_refresh_nation_player_pool_cache_batch(40, false);
  END LOOP;
...
GRANT EXECUTE ON FUNCTION public.international_refresh_nation_player_pool_cache_batch(integer, boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.international_refresh_nation_player_pool_cache() TO authenticated;
GRANT EXECUTE ON FUNCTION public.international_player_pool_rating_band(text) TO authenticated;
