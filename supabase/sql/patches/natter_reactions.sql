...
CREATE OR REPLACE FUNCTION public.natter_is_allowed_reaction_emoji(p_emoji text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT p_emoji = ANY(public.natter_allowed_reaction_emojis());
$$;
...
ALTER TABLE public.natter_reactions DROP CONSTRAINT IF EXISTS natter_reactions_emoji_check;
...
ALTER TABLE public.natter_reactions ADD CONSTRAINT natter_reactions_emoji_check
  CHECK (public.natter_is_allowed_reaction_emoji(emoji));
...
BEGIN
  ...
  IF v_emoji IS NULL OR NOT public.natter_is_allowed_reaction_emoji(v_emoji) THEN
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'reaction_emojis', to_jsonb(public.natter_allowed_reaction_emojis()),
    'posts', coalesce((
...
GRANT EXECUTE ON FUNCTION public.natter_allowed_reaction_emojis() TO authenticated;
GRANT EXECUTE ON FUNCTION public.natter_is_allowed_reaction_emoji(text) TO authenticated;
