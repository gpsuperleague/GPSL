-- =============================================================================
-- Transfer news ticker active months = live transfer windows:
--   June, July, August, January
--
-- Each UK calendar day: highest-fee deals that day (draft + free market),
-- padded from the current GPSL month window to max 5.
--
...
BEGIN
  ...
  FOR v_row IN
    ...
    WHERE coalesce(h.transfer_sale_note, '') NOT IN (
      ...
      'emergency_loan', 'emergency_loan_return'
    )
