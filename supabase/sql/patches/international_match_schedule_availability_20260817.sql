-- =============================================================================
-- International kick-off scheduling — same availability rules as league/cup
--
-- Slot picker from owner weekly availability ∩ GPSL month window.
-- Home nation proposes first; opponent accepts (or auto-agree if vacant).
--
-- Safe re-run. Requires match scheduling patches + WC engine.
-- =============================================================================

-- Proposal window from international fixture season + gpsl_month
CREATE OR REPLACE FUNCTION public.international_match_schedule_proposal_window(
  p_fixture_id bigint
)
RETURNS TABLE (
...
-- Proposer’s available 30-min UK slots in the fixture month window
CREATE OR REPLACE FUNCTION public.international_match_schedule_club_window_slots(
  p_fixture_id bigint,
...
BEGIN
  ...
  IF NOT FOUND OR v_fix.played THEN
    RETURN;
  END IF;
  ...
  LOOP
    IF v_cursor > v_now
       ...
       AND public.match_schedule_club_available_at(
             v_fix.season_id, v_club, v_cursor
           )
       AND NOT public.match_schedule_club_busy_at(
             v_club, v_cursor, NULL, v_fix.season_id
           )
    THEN
      kickoff_at := v_cursor;
      RETURN NEXT;
...
-- Context for slot-picker UI (shape aligned with club schedule context)
...
-- Propose / counter using owner availability (replaces free-form datetime)
...
BEGIN
  ...
  -- Vacant opponent → auto-agree (testing / filler nations)
  ...
  IF v_opp_club IS NOT NULL THEN
    PERFORM public.owner_inbox_send(
      ...
      CASE WHEN NOT v_is_counter THEN 'International kickoff proposed' ELSE 'International counter-proposal' END,
...
GRANT EXECUTE ON FUNCTION public.international_match_schedule_proposal_window(bigint) TO authenticated;
GRANT EXECUTE ON FUNCTION public.international_match_schedule_club_window_slots(bigint, text) TO authenticated;
