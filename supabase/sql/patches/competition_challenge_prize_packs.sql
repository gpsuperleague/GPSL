...
-- Then run competition_challenge_prize_packs_part2.sql.
-- Safe re-run.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- Tables FIRST (so later functions always compile even if earlier steps fail)
-- ---------------------------------------------------------------------------
...
-- Soft-add FK to suspensions when that table exists (avoid aborting whole script)
DO $appeals_fk$
BEGIN
  IF to_regclass('public.competition_player_suspensions') IS NOT NULL
     AND NOT EXISTS (
       SELECT 1 FROM pg_constraint
       WHERE conname = 'competition_suspension_appeals_suspension_id_fkey'
     ) THEN
    ALTER TABLE public.competition_suspension_appeals
      ADD CONSTRAINT competition_suspension_appeals_suspension_id_fkey
      FOREIGN KEY (suspension_id)
      REFERENCES public.competition_player_suspensions (id)
      ON DELETE CASCADE;
  END IF;
END;
$appeals_fk$;

CREATE UNIQUE INDEX IF NOT EXISTS competition_suspension_appeals_pending_uidx
...
DO $bonus_amount$
BEGIN
  IF to_regclass('public.competition_challenge_period_bonus_awarded') IS NULL THEN
    RAISE NOTICE 'competition_challenge_period_bonus_awarded missing — run competition_challenges.sql first';
    RETURN;
  END IF;
...
-- (Tables / RLS / seed packs created at top of this file)
