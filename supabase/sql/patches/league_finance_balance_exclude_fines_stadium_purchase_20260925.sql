-- =============================================================================
-- League finance balance: exclude fines + stadium purchases from ops P&L
--
-- Season ecosystem check already excludes transfers + loans.
-- Also exclude:
--   • gov_fine_compensation  (match / discipline fines)
--   • infra_purchase         (starting stadium buy — one-off, not day-to-day ops)
--
-- Taxes, stadium maintenance / expansions stay in ops.
-- Amounts still returned in category totals / per-club rows for visibility.
--
-- Safe re-run. After apply: Admin → Bills & Income → League finance balance → Run.
-- =============================================================================
...
BEGIN
  ...
  LOOP
    ...
    SELECT
      ...
      -- Stadium ops: maintenance / expansion / etc. (purchase excluded below)
      coalesce(sum(CASE
      ...
      -- Taxes only (fines excluded below)
      coalesce(sum(CASE
    ...
    -- Residual must also subtract excluded buckets so they don't land in other_ops
    v_other_ops :=
      v_ledger_net
    ...
    -- Ops net: day-to-day only (no transfers, loans, fines, stadium purchases)
    v_ops_net :=
      v_gates + v_prizes + v_tv + v_subsidies + v_wages + v_stadium
...
COMMENT ON FUNCTION public.competition_admin_league_finance_balance(bigint, numeric) IS
  'Admin league P&L snapshot: ops net excludes transfers, loans, fines, and stadium purchases; compares average to target profit.';
