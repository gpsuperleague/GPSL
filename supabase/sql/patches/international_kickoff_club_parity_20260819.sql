-- Align international kick-off negotiation with club league/cup:
--   Home proposes first.
--   Opponent (home or away) may Accept OR Counter from their own availability.
-- Replaces international_kickoff_home_only_20260818.sql rules.
-- Keeps withdraw + proposal history.
-- Safe re-run.
...
BEGIN
  ...
  -- Same as club: slots from YOUR owner availability in the GPSL month window
...
BEGIN
  ...
  BEGIN
    PERFORM public.owner_inbox_send(
      ...
      END,
      format(
        E'%s\nProposed kick-off: %s\nOpen International Matchday to accept or counter (same as league/cup).',
