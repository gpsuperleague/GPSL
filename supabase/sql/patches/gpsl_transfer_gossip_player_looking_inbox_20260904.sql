-- =============================================================================
-- Discord gossip: also notify holding club when player "is interested in" a club
-- (drama copy — player→club angle). Safe re-run.
-- =============================================================================
...
CREATE OR REPLACE FUNCTION public.owner_inbox_notify_transfer_gossip_interest(
  ...
  p_angle text DEFAULT 'club_to_player'
)
RETURNS bigint
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_angle text := lower(nullif(btrim(coalesce(p_angle, '')), ''));
  v_holding text;
  v_team text;
  v_id bigint;
  v_dedupe text;
...
BEGIN
  ...
  IF v_angle IS NULL OR v_angle NOT IN ('club_to_player', 'player_to_club') THEN
  ...
  v_dedupe := format(
    ...
    coalesce(
      ...
      p_rumour_id::text,
      v_player_id || ':' || v_interest_short || ':' || v_angle || ':' || to_char(now(), 'YYYYMMDDHH24')
    )
  );

  IF v_angle = 'player_to_club' THEN
    v_title := format('Player interest — %s', v_player_name);
    v_body := format(
      'Word is %s has his eye on %s. He''s contracted to you — might be nothing, might be drama. Worth a quiet word.',
      v_player_name,
      v_interest_name
    );
  ELSE
    v_title := format('Transfer interest — %s', v_player_name);
    v_body := format(
...
-- Drop old 8-arg overload if present (new signature adds p_angle)
DROP FUNCTION IF EXISTS public.owner_inbox_notify_transfer_gossip_interest(
  text, text, text, text, bigint, text, bigint, text
);

COMMENT ON FUNCTION public.owner_inbox_notify_transfer_gossip_interest(
  text, text, text, text, bigint, text, bigint, text, text
) IS
  'Inbox the holding club for Discord gossip (club→player interest or player→club wanderlust).';
...
BEGIN
  ...
  -- Holding club gets drama either way (rival interest OR player looking elsewhere)
