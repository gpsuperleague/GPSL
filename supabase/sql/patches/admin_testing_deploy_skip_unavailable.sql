-- =============================================================================
-- Admin testing deploy: skip injured / suspended players when building XIs
--
-- Fixes: "Player X is unavailable for this match (Injured/Suspended…)"
-- when Deploy Test Month randomly picks unavailable players into the XI.
--
-- Run in Supabase SQL Editor, then retry February deploy.
-- =============================================================================

CREATE OR REPLACE FUNCTION public.admin_testing_club_available_count(
  p_club text,
  p_fixture_id bigint
)
RETURNS int
...
AS $$
  SELECT CASE
    WHEN p_fixture_id IS NULL THEN
      public.club_squad_player_count(p_home_club) >= 11
      AND public.club_squad_player_count(p_away_club) >= 11
    ELSE
      public.admin_testing_club_available_count(p_home_club, p_fixture_id) >= 11
      AND public.admin_testing_club_available_count(p_away_club, p_fixture_id) >= 11
  END;
$$;
...
-- Redeploy helper: pass fixture id into XI builder + availability-aware squad check
...
-- Preview: treat injured/suspended as blocking when <11 available
...
BEGIN
  ...
  FROM (
    SELECT
      ...
      f.status,
      f.competition_type,
      public.admin_testing_club_available_count(f.home_club_short_name, f.id) AS home_squad_size,
      public.admin_testing_club_available_count(f.away_club_short_name, f.id) AS away_squad_size,
      (
        f.status = 'scheduled'
      ...
      CASE
        ...
        WHEN public.admin_testing_club_available_count(f.home_club_short_name, f.id) < 11 THEN
          format(
            'home %s has %s available (need 11; injured/suspended excluded)',
            f.home_club_short_name,
            public.admin_testing_club_available_count(f.home_club_short_name, f.id)
          )
        WHEN public.admin_testing_club_available_count(f.away_club_short_name, f.id) < 11 THEN
          format(
            'away %s has %s available (need 11; injured/suspended excluded)',
            f.away_club_short_name,
            public.admin_testing_club_available_count(f.away_club_short_name, f.id)
          )
        ELSE 'ready'
      END AS block_reason,
...
-- Month deploy remaining_ready uses fixture-aware availability
...
GRANT EXECUTE ON FUNCTION public.admin_testing_club_available_count(text, bigint) TO authenticated;
GRANT EXECUTE ON FUNCTION public.admin_testing_fixture_squads_ready(text, text, bigint) TO authenticated;
