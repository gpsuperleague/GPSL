-- =============================================================================
-- Season 1 Close Finances lines → finance archive
-- Covers: wages, 34+, star tax, manager salary, debt interest, FFP, balance interest
--
-- Same root cause as wages: Archive ran before Close Finances, so the Season 1
-- snapshot missed EOS lines. Also: eos_balance_interest was missing from the
-- finances UI type map (client fix ships separately).
--
-- Safe: restores ledger from charge_paid without re-debiting cash; moves
-- mis-tagged current-season rows back; re-snapshots archive (keeps original
-- opening/closing for past seasons).
-- Safe re-run. Paste the diagnose grid if still blank.
-- =============================================================================
...
BEGIN
  ...
  LOOP
    ...
    IF v_is_current THEN
      SELECT cf.balance INTO v_closing
    ...
    INSERT INTO public.competition_club_finance_season_archive (
      season_id, season_label, club_short_name,
      opening_balance, closing_balance, income_total, cost_total, net_total, ledger_lines
    )
    VALUES (
      v_season.id, v_season.label, v_club,
      v_opening, v_closing, v_income, v_cost, v_net, v_lines
    )
...
CREATE OR REPLACE FUNCTION public.admin_diagnose_season_close_finances_archive(
...
BEGIN
  ...
  RETURN QUERY
  SELECT 'ledger'::text, l.entry_type::text, count(*)::bigint
  FROM public.competition_finance_ledger l
  WHERE l.season_id = v_sid AND l.entry_type = ANY (v_types)
  ...
  WHERE p.season_id = v_sid AND p.charge_type = ANY (v_types)
  GROUP BY p.charge_type
  ORDER BY p.charge_type;

  RETURN QUERY
  SELECT 'archive_json'::text, e.entry_type, count(*)::bigint
  FROM public.competition_club_finance_season_archive a
  CROSS JOIN LATERAL (
    SELECT nullif(x.elem->>'entry_type', '') AS entry_type
    FROM jsonb_array_elements(coalesce(a.ledger_lines, '[]'::jsonb)) AS x(elem)
  ) e
  WHERE a.season_id = v_sid AND e.entry_type = ANY (v_types)
  GROUP BY e.entry_type
  ORDER BY e.entry_type;

  RETURN QUERY
  SELECT 'on_current_season'::text, l.entry_type::text, count(*)::bigint
  FROM public.competition_finance_ledger l
...
GRANT EXECUTE ON FUNCTION public.admin_diagnose_season_close_finances_archive(text) TO authenticated;
...
BEGIN
  ...
  IF v_sid IS NULL THEN
    RAISE EXCEPTION 'Could not find Season 1';
  END IF;
  ...
  SELECT count(*)::int INTO v_ledger_n
  FROM public.competition_finance_ledger l
  WHERE l.season_id = v_sid AND l.entry_type = ANY (v_types);

  SELECT count(*)::int INTO v_paid_n
  FROM public.competition_season_charge_paid p
  WHERE p.season_id = v_sid AND p.charge_type = ANY (v_types);

  RAISE NOTICE 'BEFORE Season % id=% | ledger close-fin lines=% | charge_paid=% | current=%',
    v_label, v_sid, v_ledger_n, v_paid_n, v_current;

  FOR v_paid IN
    SELECT *
    FROM public.competition_season_charge_paid p
    WHERE p.season_id = v_sid
      AND p.charge_type = ANY (v_types)
      AND NOT EXISTS (
  ...
  LOOP
    ...
    v_desc := CASE v_entry
      ...
      WHEN 'eos_debt_interest' THEN 'End of season debt interest (archive repair)'
      WHEN 'eos_ffp_charge' THEN 'FFP charge (archive repair)'
      WHEN 'eos_balance_interest' THEN 'End of season balance interest (archive repair)'
      ELSE v_entry
    END;

    -- Balance interest is a credit; everything else is a debit
    IF v_entry = 'eos_balance_interest' THEN
      v_amount := abs(coalesce(v_paid.amount, 0));
    ELSE
      v_amount := -abs(coalesce(v_paid.amount, 0));
    END IF;
    ...
    VALUES (
      v_sid, NULL, v_paid.club_short_name, v_entry, v_amount, v_desc,
  ...
  IF v_current IS NOT NULL AND v_current <> v_sid THEN
    FOR v_row IN
      ...
      WHERE l.season_id = v_current
        ...
        AND NOT EXISTS (
          SELECT 1 FROM public.competition_finance_ledger x
  ...
  SELECT count(*)::int INTO v_archive_n
  FROM public.competition_club_finance_season_archive a
  CROSS JOIN LATERAL jsonb_array_elements(coalesce(a.ledger_lines, '[]'::jsonb)) x(elem)
  WHERE a.season_id = v_sid
    AND x.elem->>'entry_type' = ANY (v_types);

  RAISE NOTICE 'AFTER inserted=% moved=% archive_clubs=% | ledger=% | archive_json=%',
    v_inserted, v_moved, v_archived, v_ledger_n, v_archive_n;
END;
$repair$;

SELECT * FROM public.admin_diagnose_season_close_finances_archive('1');
