...
BEGIN
  IF NEW.draft_auction_start_time IS DISTINCT FROM OLD.draft_auction_start_time
    ...
    v_finish := CASE
      WHEN NEW.draft_random_finish_time IS NOT NULL THEN
        to_char(NEW.draft_random_finish_time AT TIME ZONE 'Europe/London', 'Dy DD Mon YYYY HH24:MI')
      ELSE 'TBC'
    END;

    PERFORM public.owner_inbox_notify_all_clubs(
      'draft_scheduled',
      'Draft auction scheduled',
      format(
        E'Player/manager draft auction opens %s (UK).\nRandom finish: %s.\nCheck the draft auction pages and calendar.',
        v_start, v_finish
      ),
      'draftauction.html',
      'draft_scheduled:' || NEW.draft_auction_start_time::text,
      NULL
    );
  END IF;
