-- =============================================================================
-- Public read for active injuries (club.html / squad badges)
--
-- Admin injury list uses SECURITY DEFINER RPCs, so it works even when RLS
-- blocks table SELECT. Club pages were reading competition_player_injuries
-- directly and got zero rows when RLS was enabled without a policy.
-- Safe re-run.
-- =============================================================================

ALTER TABLE public.competition_player_injuries ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.competition_player_injury_fixtures ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS competition_player_injuries_select_authenticated
  ON public.competition_player_injuries;
CREATE POLICY competition_player_injuries_select_authenticated
  ON public.competition_player_injuries
  FOR SELECT
  TO authenticated
  USING (true);

DROP POLICY IF EXISTS competition_player_injury_fixtures_select_authenticated
  ON public.competition_player_injury_fixtures;
CREATE POLICY competition_player_injury_fixtures_select_authenticated
  ON public.competition_player_injury_fixtures
  FOR SELECT
  TO authenticated
  USING (true);
...
-- Ensure club squad RPC allows any club (public club pages)
