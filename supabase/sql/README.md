...
**GPDB nation sync / selectable set:** Run [`patches/international_sync_gpdb_nations.sql`](./patches/international_sync_gpdb_nations.sql), [`patches/international_nations_seed_rank_expand.sql`](./patches/international_nations_seed_rank_expand.sql) (required before sync — old schema capped seed rank at 99), [`patches/international_nation_player_pool.sql`](./patches/international_nation_player_pool.sql), then [`patches/international_refresh_selectable_and_seed_ranks.sql`](./patches/international_refresh_selectable_and_seed_ranks.sql). Admin → **Refresh selectable nations** (imports GPDB labels, rebuilds pool cache, activates only squad-viable nations) then **Recompute seed ranks from pool** (orders active nations by rating-band strength for qualifying pots).

**Admin:** GPSL Admin → **World Cup & nations** (`admin_international.html`) — refresh selectable nations, recompute seed ranks, open nation selection, assign teams, **skip current pick** if an owner is slow (run [`patches/international_admin_skip_pick.sql`](./patches/international_admin_skip_pick.sql)).
