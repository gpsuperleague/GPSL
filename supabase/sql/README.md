...
**UI:** Transfers → **Manager Database** (`MGDB.html`), **Manager Market** (`manager_listings.html`). Admin → **Manager contract targets**, **Transfer window** (manager draft toggle). Manager draft uses the **same 7pm UK schedule** as player draft — run [`patches/managers_draft_schedule.sql`](./patches/managers_draft_schedule.sql) and re-save transfer settings if countdown is missing.
